	...
	SurfaceGraph& meta_graph)
{
	...
	{
		if (orig_node_has_beeen_merged[merge_pair.first] || orig_node_has_beeen_merged[merge_pair.second])
		{
			continue;
		}
		...
		Vec3d meta_normal = (first_node.area * first_node.normal + second_node.area * second_node.normal).normalized();
