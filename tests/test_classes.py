...
from agplib.crossfield import VertexWithNormal, SurfaceGraphNode, SurfaceGraph, CrossConstraint

def test_tangent_space():
    test_space_1 = TangentSpace()
    test_space_2 = TangentSpace(np.array([0,1,0]), np.array([1,0,0]))
    test_space_3 = TangentSpace(np.array([0,1,0]), np.array([1,0,0]), np.array([1,1,1]))
    print("Created tangent spaces:", type(test_space_1), type(test_space_2), type(test_space_3))

def test_vertex_with_normal():
    test_vertex = VertexWithNormal(np.array([1,2,0]), np.array([1,1,1]))
    print("Created vertex with normal:", type(test_vertex))

def test_surface_graph_node():
    test_area = 1.0
    test_normal = np.array([0,0,1])
    test_node = SurfaceGraphNode(test_area, test_normal)
...
def test_surface_graph():
    ...
    node_id_0 = surface_graph.add_node(SurfaceGraphNode(1.0, np.array([0,0,1])))
    node_id_1 = surface_graph.add_node(SurfaceGraphNode(1.0, np.array([0,0,1])))
...
def test_cross_constraint():
    ...
    test_weight = 1.0
    test_node_id = 0
    test_constraint = CrossConstraint(test_weight, test_direction, test_node_id)
