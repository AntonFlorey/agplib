...
from agplib import TangentSpace
import pytest
import numpy as np
...
def test_planar_quad_curvature():
    ...
    vertices_with_normals = [
        cf.VertexWithNormal([0,0,0], [0,0,1]),
        cf.VertexWithNormal([1,0,0], [0,0,1]),
        cf.VertexWithNormal([1,1,0], [0,0,1]),
        cf.VertexWithNormal([0,1,0], [0,0,1])
    ]

    tangent_space = TangentSpace([1,0,0], [0,1,0])

    IIff = cf.approximate_II_fundamental_form(vertices_with_normals, tangent_space)

    print("II fundamental form:", IIff)

    principal_curvature_info_v1 = cf.compute_principal_curvature(vertices_with_normals, tangent_space)
    principal_curvature_info_v2 = cf.compute_principal_curvature(vertices_with_normals, np.asarray([0,0,1]))
...
def test_curved_quad_curvature():
    ...
    vertices_with_normals = [
        cf.VertexWithNormal([0,0,0], [-np.sin(rot_angle),0,np.cos(rot_angle)]),
        cf.VertexWithNormal([1,0,0], [np.sin(rot_angle),0,np.cos(rot_angle)]),
        cf.VertexWithNormal([1,1,0], [np.sin(rot_angle),0,np.cos(rot_angle)]),
        cf.VertexWithNormal([0,1,0], [-np.sin(rot_angle),0,np.cos(rot_angle)])
    ]
